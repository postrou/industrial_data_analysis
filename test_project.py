...
class RegressionModelTestCase(unittest.TestCase):
    ...
    def test_fit_X_for_NaN_values(self):
    ...
    def test_fit_y_for_NaN_values(self):
        X = np.eye(2)
        y = np.array([0, np.nan])
    ...
    def test_predict_X_test_wrong_dimensions(self):
        X_train = np.eye(2)
    ...
    def test_predict_y_for_NaN_values(self):
        X_train = np.eye(2)
        X_test = np.array([[1, 2], [np.nan, 5]])
        y_train = np.ones(2)
